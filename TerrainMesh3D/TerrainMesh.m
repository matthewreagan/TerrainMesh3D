...
#pragma mark - Compute Normals

- (void) computeNormals {
    int squaresPerSide = (_verticesPerSide - 1);
    int totalSquares = squaresPerSide * squaresPerSide;
    int totalTriangles = totalSquares * 2;
    for (int i = 0; i < totalTriangles * 3; i += 6) {
        int toprightIndex = _triangleIndices[i];
        int topleftIndex = _triangleIndices[i + 1];
        int bottomleftIndex = _triangleIndices[i + 2];
...
- (void)configureGeometry {
    ...
    SCNGeometrySource *vertexSource = [SCNGeometrySource geometrySourceWithVertices:_meshVertices count:totalVertices];
